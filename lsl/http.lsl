...
// user
string _CHECK_ERROR = "Check error";
string _CODE_DELETED = "Code deleted";
// ============================================================
...
state run {
    link_message(integer sender_num, integer num, string str, key id) {
        ...
        else if (num == CHECK_USER) {
            updateTerminal("check_user_code", "code="+str+ARGS_SEPARATOR+"uuid="+(string)id);
        }
    }
    ...
    http_response(key request_id, integer status, list metadata, string body) {
        if ( status != 200 ) {
            ...
            string command = llList2String(data,0);
            llOwnerSay(_SYMBOL_HOR_BAR_2);
            if ( command == "check_user_code" ) {
                string success = llList2String(data,1);
                if (success == "success") {
                    llMessageLinked(LINK_THIS, USER_VALID, llList2String(data,2), NULL_KEY);
                }
                else if (success == "error") {
                  llMessageLinked(LINK_THIS, USER_NOT_VALID, llList2String(data,2), NULL_KEY);
                }
            }
            else {
                llOwnerSay(body);
